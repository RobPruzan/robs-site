...
Final Code: https://github.com/RobPruzan/react-scratch
...
Another way to ask that question is, if component A re-renders, should component b re-render? Lets take a look at what our current view tree would look like:
...
This dependency tree is more inline with how react determines to re-render components. This means we have a bug, as our implementation would re-rerender `ComponentB` when a sibling in the dependency changed, because it was a child in the view tree.
...
To actually build this dependency tree, we need to know in which component a given component was defined. This works because after you call createElement, there's no way to update its props:
...
So all we have to do is track which component the `createElement` call happened in. We can do this with not too much effort:
...
> Note: This strategy is only close to what I ended up doing. I a chose a more roundabout method so it would play nicely with my existing code. The general idea of what I really did was:
>
> - take the output internal metadata by a component (a tree like structure), and flatten it into an array
> - If any node inside the array is already in the render tree, it must have been called earlier, so filter it out
...
A simple way we can use the render tree is as we are generating the view tree, only re-render a component if it depends on the component that triggered the re-render (or if it's never been rendered before)
...
When a node has the same index path between render trees, we can transfer over the state from the previous tree. If there's a new path we can't map (implying its a new node), we don't transfer any state over and let it initialize itself.
...
This was also proven at scale with the react compiler- a compiler that memorizes basically everything. They applied it to the Instagram codebase and saw no increases in memory and huge improvements to to interactivity:
...
And it was, for the most part, the same implementation as `useMemo`: <Link href="https://github.com/facebook/react/blob/0fa9476b9b9b7e284fb6ebe7e1c46a6a6ae85f27/packages/react-reconciler/src/ReactFiberHooks.js#L2801-L2813">https://github.com/facebook/react/blob/0fa9476b9b9b7e284fb6ebe7e1c46a6a6ae85f27/packages/react-reconciler/src/ReactFiberHooks.js#L2801-L2813</Link>
...
Something you may have noticed in our use context implementation is we never incremented the hooks called by one or mutated the render nodes hook array. This is because we read the context data based on an ID that is created outside of the react tree (createContext), and because useContext creates no data that needs to be persisted across renders, it only reads. So we are not limited by the order it was called.

This means we can call it conditionally with no problem. This is the same behavior react's new [`use` hook](https://react.dev/reference/react/use) has, which can be used to read context. I imagine the fact it can be called conditioanlly is for a very similar reason.

Though, I do not know what the limitation behind not being able to call useContext conditionally. Perhaps this was a quirk in the implementation, making `use` closer to their ideal implementation.
...
- implementing server-side rendering
  ...
  - We would have to find a way to map the HTML generated by the server to the view/render trees generated by the client side react. This would formally be known as hydration.
