...
Where we identify the overlap between the 2 objects, and take that as a result. But... there is no overlap? The left hand side solely has x,
...
- if statements to generate a base case are replaced with if subset checks (`extends`)
  - this looks a lot like an if statement because both sets (`RestWord`, `RestSentence`) only contain a single string literal
- splitting a sentence in a word and the rest of the sentence using destructuring is replaced with `infer` mapping over 2 sets- `${infer RestWord} ${infer RestSentence}`
...
you will most likely start get more comfortable with advanced typescript features, allowing you catch more bugs early.

This mental model is not perfect, but it holds up pretty well even with some of typescripts most advanced features.
