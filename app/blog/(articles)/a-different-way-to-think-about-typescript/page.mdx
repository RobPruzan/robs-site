...
- splitting a sentence in a word and the rest of the sentence using destructuring is replaced with `infer` mapping over 3 sets- `${infer R}${infer RestWord} ${infer RestSentence}`
