...
Something that's very useful when implementing complex features in complex projects is do as much of the work in an isolated project as possible
...
UIKit has no built in hot reloading library, so every change requires you to rebuild the app. Xcode will cache a good amount of the build for you, but on an app like Walmart, any code change required minutes to see anything update in the simulator.
...
Models tend to struggle pretty badly with grokking a large codebase. Add on top of that a complex feature request with a bad feedback loop and it's unlikely they will get anything done.
...
Luckily, Zenbu already has all the building blocks to make this happen. I already spent time to allow projects to be started in &lt;100ms with my "local serverless runtime" described in <Link href="/blog/zenbu-devlog-1">zenbu devlog #1</Link>

<TweetComponent id="1913607633153491158" />

Switching between parallel projects while maintaining state is already how zenbu works
