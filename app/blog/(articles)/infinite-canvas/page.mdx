...
that comes with graphics libaries. You can get pretty far with not a lot of canvas
code.
...
Instead of the user performing these uncomfortable set of actions, we could do this for them. Everytime the user zooms,
...
coordinates). We need the world coordinate because our mouse will not be moving relative to the screen when zooming.
...
<FileCodeEditor
  filePath="./app/blog/(articles)/infinite-canvas/snippets/dpi.ts"
  noCheck
/>
...
Bun will automatically send the 101 Upgrade response when we call `server.upgrade(req)`, and provides easy access
of the TCP connection to send and recieve websocket frames through the `websocket` property on `Bun.serve`

We can run the server with:

`bun run --hot <filename>`

Because bun can transpile typescript internally, we only need the single typescript file. But if you want autocomplete through typescript on the
`Bun` global, first setup a bun project with:

`bun init`

This is enough to give your IDE the information about bun's types.

Now, lets setup some interaction on our local canvas before we start recieving interactions from other players.
...
Here we use our function we made previously: `coordinatesFromMouseEvent`. Where we take in the mouses's screen coordinates, that the browser provides
in the click event, and translates + scale the coordinates based on the camera's current state. Having world coordinates will allow us to make it
very easy to share state between multiple players.

Now we are going to update our web-socket server to handle recieving and sending messages from connected clients.

Our WS server is going to do little work. The main task will be maintaining the sockets within an array,
so when users send messages to our WS server we can broadcast the message to everyone who is connected to our WS server:
...
- It's useful to setup WS event listeners in their own useEffects, it can be bug prone
  when complexity increases to have several event ws listener setups in a single useEffect.
...
- This is a very important step. When our component unmounts the websocket connection will not close automatically. And if
  the depenencies in our event listener `useEffect`s change, we will adding **more** event listeners, without ever removing the previously
  added listeners. This can be very problamatic because the functions set in the even listeners may no longer be correct- due to the
...
and it will automatically build a valid websocket frame, and then send it to the open TCP connection we made without our `bun` server.

Because we want to share all the actions we perform on our canvas with other players, lets send everything we set to our `geometry` state
to the WS server:
...
Most of this logic is pretty general, and distracts us from what we are really doing inside
our infinite canvas component. Lets generalize it and move the logic to a seperate hook:
...
Using this hook, we can implement our multiplayer infinite canvas component in a very simple manner
...
Now in \<100 lines of code, after our reusable abstractions, we have a multiplayer infintie canvas
that also supports panning and zooming for free.

### Bonus

If we were to turn on the [react compiler](https://react.dev/learn/react-compiler), we write this component is ~60 lines of code, while stil being performant!
Thats because we were using useCallback to get a stable function reference on our event handlers to avoid excessive re-draws/event handler setup/teardowns.
The react compiler can stablize our function referneces for us, letting us not have to think about our function reference changing between renders even if the
...
We now have a working multiplayer infintite canvas, along with reusable hooks for any implementation you need.
